...
def get_user_subscriptions(user_id):
    ...
    for row in c.fetchall():
        ...
        else: expires_date = expires_at
        formatted_date = expires_date.strftime("%d.%m.%Y")
        channel_name = CHANNELS[channel_type]["name"] if channel_type in CHANNELS else f"Канал ({channel_type})"
        subs.append((channel_name, formatted_date))
    conn.close()
    return subs
...
def create_premium_keyboard(user_id):
    ...
    if bal >= ch["price_stars"]:
        kb.append([{"text":f"⭐ Оплатить {ch['price_stars']} звёзд","callback_data":"pay_from_balance"}])

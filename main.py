...
crypto_prices = {"BTC": 60000, "ETH": 3200, "TON": 2.5, "USDT": 1.0}  # исправленная цена TON
...
def update_user_balance(user_id, amount, username="", first_name=""):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("""INSERT OR REPLACE INTO users (user_id, username, first_name, balance) VALUES (?, ?, ?, COALESCE((SELECT balance FROM users WHERE user_id= ?),0)+?)""",
...
def update_crypto_prices_loop():
    ...
    while True:
        try:
            ...
            print("Error updating crypto prices:", e)
        time.sleep(300)

def get_crypto_amounts(price_usd):
    return {"BTC": round(price_usd/crypto_prices["BTC"],6), "ETH": round(price_usd/crypto_prices["ETH"],4), "TON": round(price_usd/crypto_prices["TON"],2), "USDT": round(price_usd,2)}
...
def handle_callback(cb):
    ...
    if data=="channel_free":
        ...
        if invoice:
            ...
            if not inv_id:
                send_message(chat_id,f"❌ Не найден ID инвойса для {cur}")
                return
            active_crypto_invoices[inv_id] = {"user_id": user_id,"chat_id": chat_id,"created_at": time.time(),"duration_days": ch["duration_days"]}
            pay_url = invoice.get("pay_url","ссылка недоступна")
            send_message(chat_id,f"💎 Оплата {cur}:\n🔗 {pay_url}")
        else:

...
def create_crypto_invoice(amount_usd, currency="USDT", description="Подписка"):
    ...
    rates = {"USDT":1.0,"TON":0.5,"BTC":0.000004,"ETH":0.00003}  # пример курсов
    if currency not in rates:
        return None
    amt = round(amount_usd / rates[currency], 6)
    payload = {
        "asset": currency,
        "amount": str(amt),
        "description": description,
